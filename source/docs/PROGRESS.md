...
**Fase atual:** 🚀 EM PRODUÇÃO · **2026-06-13**: Portal do responsável (token 90 dias, sem login), MFA TOTP via Supabase, plano de aulas por dia vinculado ao motor de previstas — tudo entregue (typecheck + build verdes, commit cedbac9). Próximo: push para Vercel. · **Console de admin redesenhado 2026-06-04** (overview + sidebar + /admin/contas + detalhe por escola; acesso ao app via "Entrar como"). · **Storage ATIVADO 2026-06-04** (ADR 0005, 2 buckets): Fatia 1 = upload da logo entregue; falta `SUPABASE_SERVICE_ROLE_KEY` no Vercel; próximo = Fatia 2 materiais (3.1/3.3) → depois RAG no EduON. · **Frente Qualidade ("melhor versão") ENCERRADA 2026-06-04**: entregue o que tem ROI real — Q1 (feedback de submit, 35 telas) + Q2 (`<KpiCard>`) em prod; Q6 (empty states já consistentes + fix lixeira) e Q8 (loading global já cobre tudo) confirmados; Q5 (PageHeader) já amplo; Q3/Q4/Q7 deferidos por baixo ROI (tabelas/forms heterogêneos; aria-label cosmético). RBAC já estava plugado (assertCan na camada de serviço — falso positivo da auditoria). · Sequência autônoma 2026-06-03: vínculos prof. (17), faltas/matéria + doc PDF (8/8.1/8.2/9.1), import CSV, menu enxuto (18 parcial), matérias da turma + série/idade (3/3.2), vínculo responsável (4/5), notas participação/anotação (9), cronograma (7), quadro de funcionários (1 parcial), Meu padrão EduON (18.3), painel da escola + gráficos (14/15 parcial), PWA + nav mobile (16). · **Status:** EM ANDAMENTO · **Próximo passo (nova sessão):** itens que dependem do Lucas — Storage (materiais 3.1/3.3 + RAG 11.2-11.4), Stripe (billing), WhatsApp Cloud API; e BNCC (dados). Restantes sem credencial: plano de aulas (7.1), mural dos pais (12), banco coletivo (13). Prod: https://on-education-seven.vercel.app

---

## Log de checkpoints

### [2026-06-13] — Portal do responsável + MFA TOTP + Plano de aulas por dia — STATUS: CONCLUÍDO

- **guardian_access_tokens** (migration `0043`, aplicada em prod): SHA-256 hash, expiresAt 90 dias, lastUsedAt. RLS isolada por tenant.
- **Portal público `/portal/[token]`**: sem login, exibe notas/faltas/comunicados de cada aluno vinculado. Invalida link expirado com mensagem clara.
- **Botão "Portal"** em `/app/escola/responsaveis`: gera token + redireciona para a mesma página com o link raw em banner verde (exibido uma vez).
- **MFA TOTP** via `supabase.auth.mfa` (sem migration extra): página de enrollment `/app/conta/mfa` (QR + secret + verificação + remoção); página de desafio `/mfa-challenge?next=` (listFactors → challenge → verify); middleware redireciona AAL1 → `/mfa-challenge` quando `nextLevel=aal2`.
- **Plano de aulas por dia** (`/app/sala/plano-diario`): lista previstas dos próximos 14 dias por turma; por aula: vincular plano existente ou gerar via WayOn (auto-linka ao plano gerado). `listUpcomingLessons` + `linkLessonToPlan` no module-sala-de-aula; `linkLessonToPlanAction` + `generateLessonPlanAction` atualizado.
- **Nav:** item "Plano de aulas" (CalendarClock) + "Segurança (MFA)" (ShieldCheck) adicionados.
- **Arquivos principais:** `packages/db/drizzle/0043_*.sql`, `packages/modules/nucleo/src/portal.ts`, `apps/web/src/app/portal/[token]/page.tsx`, `apps/web/src/app/app/conta/mfa/page.tsx`, `apps/web/src/app/mfa-challenge/page.tsx`, `apps/web/src/app/app/sala/plano-diario/page.tsx`, `apps/web/src/middleware.ts`, `apps/web/src/lib/supabase-browser.ts`.
- **Testes:** `typecheck` · `build` — verdes (14/14).
- **Commit(s):** `cedbac9 feat: portal do responsável, MFA TOTP, plano de aulas por dia`.
