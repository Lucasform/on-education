...
- [x] Simulados/quizzes simples — múltipla escolha com correção automática, tentativas e nota por aluno. `module-pedagogico` (`quizzes.ts`), tabelas `quizzes`/`quiz_questions`/`quiz_attempts` (migration `0007`, RLS), páginas `/app/simulados` e `/app/simulados/[id]`.
...
- [~] Onboarding assistido: cria tenant `organization`, unidades, convida membros (RBAC completo). — `provisionOrganizationTenant` (owner+director, plano `school_starter`, unidade Sede), `createUnit/listUnits`, `inviteMember/acceptInvitation/listInvitations`. Falta UI de onboarding.
