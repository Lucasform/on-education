...
- [x] **9. Notas: participação, formais e anotações por aluno** — `grades.kind` (formal/participacao/anotacao) + `grades.note`; `value` agora nulo p/ anotação; médias (boletim/relatórios/aluno) ignoram anotações.
